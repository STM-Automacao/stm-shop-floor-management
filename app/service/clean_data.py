...
class CleanData:
    ...
    def clean_maq_cadastro(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        --------
            ...
            >>> import pandas as pd
            >>> df = pd.DataFrame({'maquina_id': [TMF001, TMF002, TMF003], 'linha': [1, 2, 3], 'fabrica': [1, 2, 2], 'data_registro': ['2021-01-01', '2021-01-01', '2021-01-01'], 'hora_registro': ['00:00:00.000', '00:00:00.000', '00:00:00.000'], 'recno': [1, 2, 3], usuario_id: [00532, 00533, 00534]})
        ...
        self.df = self.df.drop_duplicates(subset=["data_registro", "linha"], keep='first')

        # Criar nova coluna combinando data e hora
        self.df['data_hora_registro'] = self.df['data_registro'].astype(str) + ' ' + self.df['hora_registro'].astype(str).str.split('.').str[0]

        # Converter coluna data_hora_registro para datetime
        self.df['data_hora_registro'] = pd.to_datetime(self.df['data_hora_registro'], format='%Y-%m-%d %H:%M:%S')

        # Remover colunas desnecessárias
        self.df = self.df.drop(columns=['recno', 'data_registro', 'hora_registro'])

        # Ordenar dataframe para facilitar trabalho futuro
        self.df = self.df.sort_values(by=['maquina_id', 'data_hora_registro'], ascending=[True, False])
